...
let success_url= 'http://localhost:4001/dashboard.html';
let activeUrl = getOnlineUrlConnection();
import ApiImageUploadservice from './ApiImageUploadService';

if( document.getElementById("image-upload")){
var fileToRead = document.getElementById("image-upload");
...
fileToRead.addEventListener("change", function(event) {
    ...
    if (files.length) {
        ...
        document.getElementById('avatar-img2').src = './assets/images/' +files[0].name;
    }

     console.log('image upload target');
     ApiImageUploadservice.uploadImage(event);
...
class ApiUpdateProfile {
      ...
      .then(data => {
        ...
        if (data) {
         // MessageBoard.displayMsg('Found users profile');
          ...
          console.log('users profile unavailable');
  ...
  static updateProfile(){
    ...
    const user_type = user_typeSelect.options[user_typeSelect.selectedIndex].text;
     ...
     let avatar = document.getElementById('avatar-img2').src;
     var fullPath = avatar;
